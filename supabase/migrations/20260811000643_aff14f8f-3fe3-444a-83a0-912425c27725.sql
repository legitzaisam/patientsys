-- ENUMS
CREATE TYPE public.app_role AS ENUM ('owner','practitioner','front_desk','patient');
CREATE TYPE public.patient_status AS ENUM ('active','inactive','archived');
CREATE TYPE public.document_kind AS ENUM ('consent','treatment_plan','consultation','aftercare','other');
CREATE TYPE public.document_status AS ENUM ('draft','sent','viewed','signed','expired');
CREATE TYPE public.photo_kind AS ENUM ('before','after');
CREATE TYPE public.message_author AS ENUM ('staff','patient');
...
CREATE INDEX treatments_due_idx ON public.treatments(next_due_at);
...
CREATE INDEX treatment_photos_patient_idx ON public.treatment_photos(patient_id);
...
CREATE INDEX messages_patient_idx ON public.messages(patient_id, created_at);
GRANT SELECT, INSERT, UPDATE, DELETE ON public.messages TO authenticated;
GRANT ALL ON public.messages TO service_role;
ALTER TABLE public.messages ENABLE ROW LEVEL SECURITY;

-- MEDICAL HISTORY VERSIONS
CREATE TABLE public.medical_history_versions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  clinic_id uuid REFERENCES public.clinics(id) ON DELETE CASCADE,
  patient_id uuid NOT NULL REFERENCES public.patients(id) ON DELETE CASCADE,
...
CREATE INDEX mhv_patient_idx ON public.medical_history_versions(patient_id, created_at DESC);
...
INSERT INTO public.treatment_catalogue (clinic_id, name, category, description, price, interval_days, cooling_off_hours, requires_consent) VALUES
('11111111-1111-4111-8111-111111111111','Botulinum Toxin Type A','Injectable','Wrinkle-relaxing injections, upper or lower face',295.00,120,48,true),
('11111111-1111-4111-8111-111111111111','Hyaluronic Acid Dermal Filler','Injectable','Volume restoration and contouring',450.00,270,48,true),
('11111111-1111-4111-8111-111111111111','Polynucleotide Skin Booster','Injectable','Skin quality and hydration course',280.00,90,48,true),
('11111111-1111-4111-8111-111111111111','Medical Grade Chemical Peel','Skin','Resurfacing peel for texture and pigmentation',180.00,42,24,true),
('11111111-1111-4111-8111-111111111111','Microneedling with RF','Skin','Collagen induction with radiofrequency',350.00,42,24,true),
('11111111-1111-4111-8111-111111111111','HydraFacial','Skin','Deep cleanse, extract and hydrate',150.00,30,0,false),
('11111111-1111-4111-8111-111111111111','Aesthetic Consultation','Consultation','Full facial assessment and treatment planning',0.00,NULL,0,false);