...
> What actually happened, including two findings recorded rather than fixed, is in [../WORKLOG.md](../WORKLOG.md).

**Overview:** remove every UI claim that an email or text was sent when only an in-app portal message was written, refresh the audit against HEAD `c443aac`, and establish the work log and micro-plan conventions the remaining twelve phases depend on. No behaviour changes, no schema changes.
...
- 4.5 staff notifications: **resolved** in commit `b6c7516`, now scoped by `recipient_id` at `clinic.functions.ts:1368`.
...
- `listStaffDirectory` filters roles to `["owner","practitioner","front_desk"]` at `clinic.functions.ts:1408`, omitting `manager`, so managers can be missing from alert recipient lists. Functional bug, not just cosmetic.
