...
## 16. Phase 4 — Runtime validation

### 16.1 §4.8 — No runtime input validation. RESOLVED.

All 62 `.validator(...)` calls in both `clinic.functions.ts` and `clinic.functions.demo.ts` now parse their input against a named schema. The count in §12 was 53 at the time of the audit and 62 by [Phase 3](#15-phase-3--capability-based-rbac); zero bare `(data) => data` pass-throughs remain in either file.

Zod is no longer an unused dependency. Schemas live in `src/lib/validation/`, split into [primitives.ts](../src/lib/validation/primitives.ts) (138 lines of shared rules), [schemas.ts](../src/lib/validation/schemas.ts) (62 schemas) and [parse.ts](../src/lib/validation/parse.ts).

**Why schemas are not passed to `.validator()` directly.** TanStack Start's `execValidator` recognises Standard Schema, which zod 3.24 implements, and reports a failure by stringifying the raw issue array. Around 52 call sites do `onError: (e) => toast.error(e.message)`, so `.validator(SavePatient)` would have rendered a multi-line JSON blob in a toast. Each validator therefore keeps its function form and calls `parseInput(Schema, data)`, which flattens the `ZodError` into one sentence — "First name is required", "Duration minutes is too large (maximum 1440)" — and preserves the declared TypeScript input type at every call site.

**One source for production and demo.** The Vite plugin only rewrites `src/lib/clinic.functions.ts`, so a module under `src/lib/validation/` is never swapped. Both files import the same 62 schemas. Unlike the Phase 1 guards, there is no production/demo drift to accept here.

### 16.2 The control

`npm run check:validators`, modelled on `check-policy.mjs`, fails when a validator is still a pass-through, when a schema is unused, when a handler's production and demo validators name different schemas, or when a schema's fields do not match the fields its validator declares in TypeScript. That last check is what stops a schema silently omitting a field and waving it through — it caught exactly that during the build, where `SendDocument` was missing `treatment_id`.

### 16.3 §4.8 — The stored-XSS claim was wrong

The audit stated that `saveAppointmentNote` storing note HTML without `sanitizeNoteHtml` was "a stored-XSS path into a clinical record". That was overstated, and the correction matters more than the fix.

Appointment notes are rendered as escaped React text everywhere they appear — `patients.$id.tsx:1038` and `visit-note-chip.tsx:253`. The only `dangerouslySetInnerHTML` anywhere in `src/` is static chart CSS in `ui/chart.tsx:73`. There was no sink, so there was no live vulnerability. Severity was High; it should have been Low.

The body is now routed through `sanitizeNoteHtml` anyway, as defence-in-depth: these notes are one product decision away from the rich-text editor, and stored markup would be inherited the day that happens.

### 16.4 Residual risk

**`sanitizeNoteHtml` is a regex, not a sanitiser.** It is now on both note paths, but a regex-based HTML filter should not be trusted against a determined payload. `saveMyNote` is the one that matters: its HTML is re-parsed by Tiptap `setContent` on read, so that path has a real sink. Since the note is self-scoped, the exposure is self-XSS. Replacing this with a proper sanitiser (DOMPurify) is unfinished work, not a closed finding.

**Schemas mirror today's shapes; they do not assert formats.** No `.uuid()`, because demo fixture ids are `6-4-4-4-12`; no `.datetime()`, because `date_of_birth` is `YYYY-MM-DD` while `starts_at` is full ISO. Hardening was limited to enums, ranges, max lengths, trim and non-empty. A field the verification did not sample could still accept malformed input of the right primitive type.

**Client-side validation covers four forms, not all of them.** Validation is server-only by design in TanStack Start — the schema never runs in the browser. Inline field errors were added to `invite-staff-dialog`, `patients.index`, `treatment-catalogue-settings` and `team.$id`, which finally consume the previously orphaned `ui/form.tsx`. `quick-add-appointment` and the `schedule.tsx` booking dialog were deliberately left out: both branch into an inline new-patient path spanning two schemas and are the highest-regression forms in the app. Every other form in the app still surfaces validation failures as a toast after a round trip.

**`sendMessage` still declares `as`.** §14 noted that removing the field belonged with this phase. It is still declared, now as `z.enum(["staff", "patient"])`, because the client passes it. The handler has ignored it since Phase 2 and derives the author from the session, so the field is inert — but it is still there, and a reader could reasonably think it does something.

### 16.5 A UX observation from the verification

Driving the pilot forms surfaced a real bit of jank, worth recording even though it is not a defect. Validation runs on blur, so pressing a submit button blurs the focused field, which adds or removes a message and shifts the button between `mousedown` and `mouseup`. A real user whose pointer is on the button edge can click and have nothing happen. Reserving space for the message, or validating on submit only, would fix it.

---

## 17. Phase 5 — Database hardening

Eight items from §5 and one from §4.7, addressed in six migrations plus a query-level scoping layer. The phase divides cleanly into two kinds of work, and conflating them would have produced something that looked finished and enforced nothing.

**Binds today's traffic:** the constraints, the two triggers, the nine indexes, and the `clinic_id` predicate now written into every query. **Defence-in-depth only:** the grants and the RLS policies, which matter the day §4.1 is fixed and not before. Every request still runs on a service-role client that bypasses RLS, so a policy is a statement of intent until that changes.

### 17.1 Three corrections to §5

**§5.2 overstated the cascade.** It said deleting a patient destroys their medical record. Eight child tables do cascade, but `appointment_notes.patient_id` is `NO ACTION`, so deleting any patient with a visit note already failed on a foreign key. More importantly, no application code deletes a patient anywhere — the exposure was the `owners delete patients` policy, not a UI button. Severity was High; as an exploitable path it was Low. What was genuinely missing was a retention model, which is what this phase built.

**§4.7 understated the grants.** It named `anon`. In fact `anon` **and** `authenticated` each held all seven privileges on all 27 public tables. The one that mattered is `TRUNCATE`: RLS does not apply to `TRUNCATE` at all, so no policy could have stopped either role emptying `patients`. That was the single worst item in the phase and it was not the one the audit flagged.

**§5.5 was right about the constraint and missed the cause.** `patients.user_id` had no unique constraint, but the reason duplicates could arise is `handle_new_user`, which linked with an unbounded `UPDATE ... WHERE lower(email) = lower(NEW.email)`. Two patient rows sharing an email — a duplicate record, a couple sharing an address — would both receive the same `user_id`, and `current_patient_id()` would then return either one. Adding `UNIQUE` on its own would have converted a silent data bug into a hard signup failure, so the trigger was rewritten in the same migration.

### 17.2 What now binds the service-role client

Two triggers, verified against the service-role client rather than psql, because that is the client the application uses:

- `documents_signed_immutable` rejects any change to `body`, `title`, `kind`, `patient_id`, `status` or the four signature columns once a document is signed, and rejects deletion outright. Resending is unaffected; superseding a consent means inserting a new row. This is the highest-value item in the migration set — it closes §5.3 for real, where a policy could not have.
- `patients_no_hard_delete` refuses every `DELETE` on `patients` unless the session has set `app.allow_patient_erasure`, which only `erase_patient()` does.

### 17.3 Retention

Adopted: **8 years from the last treatment**, the NHS adult-record standard. `patients` gains `deleted_at`, `deleted_by`, `deletion_reason` and `legal_hold`; `patient_retain_until()` computes the window from the latest treatment or appointment, falling back to registration. Archived patients drop out of `listPatients` and the diary but keep their entire clinical history.

Erasure is a `SECURITY DEFINER` function, not a route. It refuses without a reason, refuses under legal hold, refuses inside the window, and writes an `audit_log` row *before* the delete so the entry outlives the record. `owners delete patients` was dropped; deletion is no longer reachable through RLS at all.

`archivePatient` is owner-only and reversible. A GDPR erasure request deliberately is not one click.

### 17.4 Tenancy — §5.1 closed at the query layer, not the policy layer

Zero of 94 policies referenced `clinic_id`. That is now 23 `RESTRICTIVE` `clinic_isolation` policies, one per clinic-scoped table, ANDed with every permissive policy — 23 statements instead of rewriting 94, and 23 `DROP POLICY` lines to revert.

But the policies are not what enforces isolation today. The service-role client ignores them. What enforces it is `clinicScoped()` in [src/lib/auth/clinic-scope.server.ts](../src/lib/auth/clinic-scope.server.ts), applied once in the session middleware: every `select`, `update` and `delete` on a clinic-scoped table gains `.eq("clinic_id", …)` and every `insert` and `upsert` is stamped with the caller's clinic. Applying it at the boundary rather than at ~170 call sites was deliberate — a filter you have to remember to write is a filter someone forgets.

The hardcoded `const CLINIC_ID` is gone. Writes now carry the caller's clinic, resolved from their staff profile or their patient record.

### 17.5 The controls

`npm run check:tenancy` joins `check:policy` and `check:validators`. It fails on a hardcoded UUID in the server functions, on more than three raw `supabaseAdmin` imports (the three that cannot reach a table), on the middleware losing its wrapper, and — the one that will earn its keep — on any table in the generated types that is in neither the scoped nor the exempt list. That last check is what catches the table someone adds in six months.

`scripts/db-snapshot.mjs` prints grants, constraints, policies, triggers, indexes and the ledger as a diffable text file. Every claim in this section was verified by diffing a before and after capture, not from memory.

`scripts/apply-migrations.mjs` is now ledger-aware. It previously replayed all 44 files on every run and recorded nothing, which is exactly how §5.4's drift happened; the missing `20260824000000` row is recorded and the ledger reads 50 of 50.

### 17.6 Verification

A second clinic was created with its own owner and patient. Through the server functions: neither clinic's patient list includes the other's, neither can read the other's patient by id, both dashboards still load, and a `savePatient` from clinic B is stamped with clinic B. Independently through RLS, with the service role dropped and a JWT claim set: clinic A's owner resolves to clinic A, sees zero of clinic Z's patients, sees their own five, and a cross-clinic insert is refused. Both tenants were removed afterwards.

Retention was exercised end to end in a rolled-back transaction: erasure refused with no reason, refused under legal hold, refused inside the window, permitted outside it, and the audit row survived the delete.

Regression: owner, manager and patient across every route with zero console errors; all Phase 2 and Phase 3 attacks still refused; the demo sweep clean across four roles. `tsc --noEmit` holds at the 52-error baseline.

### 17.7 Residual risk

**§4.1 is untouched and it is the one that matters.** The grants and the 23 isolation policies are defence-in-depth. Moving off the service-role client is the real fix and belongs with the auth work in Phase 6. Until then, the database's opinion about who may see what is advisory.

**The wrapper enforces scope; it cannot judge intent.** `check:tenancy` proves every clinic-scoped table goes through the filter. It cannot prove the filter is the right one for a query that should legitimately cross a boundary — there are none today, and the one-clinic-per-person decision means there should not be.

**`supabase_admin`'s default privileges could not be altered** from the migration connection, so a table created by Supabase itself (rather than by our migrations) would still be granted to `anon` and `authenticated`. Tables created by `postgres`, which is every migration in this repo, inherit the tightened defaults.

**`handle_new_user` still guesses the clinic** — `SELECT id FROM clinics ORDER BY created_at LIMIT 1`. It is immediately overwritten by the invite flow for staff, and single-clinic deployments never notice, but with a genuine second tenant a self-registering user would land in the wrong clinic until corrected.

**§5.6, §5.9 and §5.10 are still open.** Migrations are still not re-runnable, the 26 `select("*")` calls still leak `documents.access_token` through `getPatient` and `getMyRecord`, and the missing foreign keys are unchanged.
