...
- §4.3 — `sendMessage` still takes `as: "staff" | "patient"` from the request body ([clinic.functions.ts:1284](../src/lib/clinic.functions.ts)). Staff impersonation remains a one-line fix and remains unfixed.
- §4.4 — `signDocument` still updates by `.eq("id", data.id)` with no ownership predicate ([clinic.functions.ts:1865](../src/lib/clinic.functions.ts)). Any authenticated user can still sign any consent document by ID.
...
| **Open** | **31** | authenticated-only; no role check and no self-scoping |

This is a coarser measure than the master plan's tiering (which reports 53/10/25) because it counts self-scoping as a distinct tier rather than folding it into "guarded". Either way the actionable figure is the same order: roughly a third of handlers have no authorization decision in them at all.

The 31 open handlers, named so this is verifiable rather than a number to argue about:
...
Not all 31 are equally severe. `getCatalogue` and `listTreatmentColours` are arguably fine for any staff member. `updateClinicDetails`, `saveCatalogueItem`, `deleteColourTheme` and `deleteMessageTemplate` are clinic-configuration writes reachable by a patient's token, and belong with the §4.6 criticals.
